...
var
i,id_board:integer;
name_txt ,Descr_txt,Tr_id ,brd_id,wh_id,bear_id,path,rd_id:string;
begin
   ...
   qryaddrdstl.Sql.Add('Select MAX(Skate_ID) from Skateboard');
   qryaddrdstl.Active:=True;
   id_board:=StrToInt(qryaddrdstl.Fields[0].AsString);
   qryaddrdstl.Active:=False;
   qryaddrdstl.SQL.Clear;
   for i:=0 to (chkRidingStyles.items.Count)-1 do begin
       if (chkRidingStyles.Checked[i]) then
           begin
             rd_id:=strRidingStyles[i];
             qryaddrdstl.SQL.Text:='exec dbo.AddRdStyles @Ridingstyle_ID=:rd_id, @Skate_id=:id_board';
             qryaddrdstl.ParamByName('rd_id').AsString :=rd_id;
             qryaddrdstl.ParamByName('id_board').AsInteger := id_board;
             qryaddrdstl.ExecSQL;
             tr1.Commit;
	   end;
   end;
